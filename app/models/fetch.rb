  ...
  def save_events
    self.files_to_fetch.each do |file_name|
      ...
      Yajl::Parser.parse(events_json) do |event|
        if event["repository"]
          self.events.create(time_created: event["created_at"], repo_name: event["repository"]["name"], the_type: event["type"], repo_url: event["repository"]["html_url"])
        elsif event["repo"]
          self.events.create(time_created: event["created_at"], repo_name: event["repo"]["name"], the_type: event["type"], repo_url: event["repo"]["html_url"])
        else
        end
      end
    end
